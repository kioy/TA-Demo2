...
MISC	 = Makefile.std LICENSE.TXT rogue54.sln rogue54.vcproj rogue.spec \
	   rogue.png rogue.desktop README.txt
MISC_CY	 = terminfo/63/cygwin cyggcc_s-1.dll cygncurses-10.dll cygwin1.dll cmd_us.bat
CC	 = gcc
# FEATURES = -DALLSCORES -DSCOREFILE=\"$(SCOREFILE)\" -DLOCKFILE=\"$(LOCKFILE)\"
FEATURES = -DALLSCORES -DMASTER -DSCOREFILE=\"$(SCOREFILE)\" -DLOCKFILE=\"$(LOCKFILE)\"
CPPFLAGS =
CFLAGS	 = -O3 -w
LDFLAGS  =
LIBS	 = -lcurses
...
#	$(RM) core a.exe a.out a.exe.stackdump $(PROGRAM) $(PROGRAM).exe $(PROGRAM).lck
	$(RM) core a.exe a.out a.exe.stackdump $(PROGRAM).lck
...
dist.src:
	$(MAKE) $(MAKEFILE) clean
	mkdir $(DISTNAME)
	cp $(CFILES) $(HDRS) $(MISC) $(AFILES) $(PROGRAM)_jp.doc $(DISTNAME)
...
dist.irix:
	...
	tar cf $(DISTNAME)-irix.tar $(PROGRAM) LICENSE.TXT README.txt $(DOCS)
	gzip -f $(DISTNAME)-irix.tar

dist.aix:
	$(MAKE) $(MAKEFILE) clean
	$(MAKE) $(MAKEFILE) CC=xlc CFLAGS="-qmaxmem=16768 -O3 -qstrict" $(PROGRAM)
	tar cf $(DISTNAME)-aix.tar $(PROGRAM) LICENSE.TXT README.txt $(DOCS)
	gzip -f $(DISTNAME)-aix.tar

dist.linux:
	$(MAKE) $(MAKEFILE) clean
	$(MAKE) $(MAKEFILE) $(PROGRAM)
	tar cf $(DISTNAME)-linux.tar $(PROGRAM) LICENSE.TXT README.txt $(DOCS)
	gzip -f $(DISTNAME)-linux.tar

dist.interix:
	@$(MAKE) $(MAKEFILE) clean
	@$(MAKE) $(MAKEFILE) CFLAGS="-ansi" $(PROGRAM)
	tar cf $(DISTNAME)-interix.tar $(PROGRAM) LICENSE.TXT README.txt $(DOCS)
	gzip -f $(DISTNAME)-interix.tar

dist.cygwin:
	@$(MAKE) $(MAKEFILE) --no-print-directory clean
	@$(MAKE) $(MAKEFILE) CPPFLAGS="-I/usr/include/ncurses" --no-print-directory $(PROGRAM)
	tar cf $(DISTNAME)-cygwin.tar $(PROGRAM).exe $(MISC_CY) LICENSE.TXT README.txt $(DOCS)
...
dist.mingw32:
	...
	zip $(DISTNAME)-mingw32.zip $(PROGRAM).exe LICENSE.TXT README.txt $(DOCS)
...
dist.djgpp:
	...
	zip $(DISTNAME)-djgpp.zip $(PROGRAM) LICENSE.TXT README.txt $(DOCS)
...
dist.win32:
	...
	zip $(DISTNAME)-win32.zip $(PROGRAM).exe LICENSE.TXT README.txt $(DOCS)
