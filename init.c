...
badcheck(const char *name, const struct obj_info *info, int bound)
{
    ...
    else if (info == pot_info)
    {
	max_pot_prob = info[bound - 1].oi_prob;
	return;
    }
    else if (info == scr_info)
    {
	max_scr_prob = info[bound - 1].oi_prob;
	return;
    }
    else if (info == ws_info)
    {
	max_ws_prob = info[bound - 1].oi_prob;
	return;
    }
