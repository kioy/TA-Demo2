...
open_score(void)
{
    ...
    FILE *psf;
    static char psfbuf[256];
    int i;

    if ((psf = fopen("lan.cfg", "r")) != NULL) {
	fgets(psfbuf, 256-2, psf);
	fclose(psf);
	for (i = strlen(psfbuf); ; i--) {
	    if (psfbuf[i] < ' ')
		psfbuf[i] = '\0';
	    else
		break;
	}
	sprintf(psfbuf, "%s%s", psfbuf, SCOREFILE);
	scorefile = psfbuf;
    }
#ifdef __CYGWIN__
    cygwin_conv_to_posix_path(scorefile, scorefile);
#endif
