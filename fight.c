...
slime_snake()
{
    ...
    if (slime_md2 == TRUE)
    {
	monsters['S'-'A'].m_name = slime_name;
	monsters['S'-'A'].m_flags |= ISSLOW;
	monsters['U'-'A'].m_name = urvile_name;
	for (i = 0; i < 26; i++)
	    strcpy(monsters[i].m_stats.s_dmg, m_stats_s_dmg[i*2+1]);
    }
    else
    {
	monsters['S'-'A'].m_name = snake_name;
	monsters['S'-'A'].m_flags &= ~ISSLOW;
	monsters['U'-'A'].m_name = bunicorn_name;
	for (i = 0; i < 26; i++)
	    strcpy(monsters[i].m_stats.s_dmg, m_stats_s_dmg[i*2]);
    }
}
...
{
    int y, x, ret = 0;
    unsigned char ch, new_sl = 0;

    for (y = tp->t_pos.y-1; y<=tp->t_pos.y+1; y++)
	for (x = tp->t_pos.x-1; x<=tp->t_pos.x+1; x++)
	{
	    if ((y==hero.y && x==hero.x) || (y<1 || y>=23 || x<0 || x>=80))
		continue;
	    if (step_ok(ch = (unsigned char)winat(y, x)))
	    {
...
/*  if (abs(tp->t_pos.y-hero.y)+abs(tp->t_pos.x-hero.x)!=1) return;  */
	...
	move(slimepos.y, slimepos.x);
	addstr("S");
    }
    tp2 = moat(slimepos.y, slimepos.x);
    if (tp2!=0)
    {
	tp2->t_flags |= ISRUN;
	tp2->t_flags &= ~ISHELD;
	tp2->t_dest = find_dest(tp2);
    }
}
...
fight(const coord *mp, const THING *weap, int thrown)
{
    ...
    if (slime_md2)
	if (tp->t_type=='S' && rnd(100)>25) slime2(tp);
    return did_hit;
}
...
attack(THING *mp)
{
	...
	mp->t_disguise = 'X';
	if (on(player, ISHALU))
	    mvaddch2(mp->t_pos.y, mp->t_pos.x, rnd(26) + 'A');
    }
    mname = set_mname(mp);
    oldhp = pstats.s_hpt;
